...
namespace Entity.Entities.Lesson
{
    ...
    public class LessonConfiguration : IEntityTypeConfiguration<Lesson>
    {
        public void Configure(EntityTypeBuilder<Lesson> builder)
        {
            ...
            builder.Property(x => x.Thumbnail).HasColumnType("nvarchar(1000)");
            ...
            builder.HasOne(x => x.LessonCategory).WithMany(x => x.Lessons).HasForeignKey(x => x.LessonCategoryId);
            builder.HasMany(x => x.Vocabularies).WithOne(x => x.Lesson).HasForeignKey(x => x.LessonId);
            builder.HasMany(x => x.Grammars).WithOne(x => x.Lesson).HasForeignKey(x => x.LessonId);
            builder.HasMany(x => x.Questions).WithOne(x => x.Lesson).HasForeignKey(x => x.LessonId);
